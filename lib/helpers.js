...
(function () {
   ...
   helpers = module.exports = {
      ...
      domify: function (f, mime) {
         ...
         var errors = [], replacer = function(m){
            errors.push(m);
         };
         //wrap(console, 'log', replacer);
         //wrap(console, 'error', replacer);
      ...
      checkFiles: function(grunt, root, sourceFile, result) {
         ...
         result.files.forEach(function(packItem, i){
            ...
            if(!fs.existsSync(fullPath)){
               ...
               errors.push(("line " + l + " col " + c + ": file not found").bold/* + s.replace(packItem, packItem.red)*/);
               isSane = false;
            }
         });
         if(!isSane) {
               ...
               results.forEach(function(result){
                  ...
                  if(result.files.length > 1) {
                     if(helpers.checkFiles(grunt, root, f, result)) {
                        ...
                        helpers.replaceCollection(dom, result, nodeProducer(dom, path.relative(root, packedFile)));
                        ...
                        grunt.log.ok("OK  : ".green + f);
                     } else {
                        grunt.log.error("Skip: ".red.bold + f);
                     }
                  }
               })
